...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S') datetime.now().strftime(f'%Y-%m-%d_%H-%M-%S')datetime.now().strftime(f'%Y-%m-%d_%H-%M-%S').datetime.now().strftime(f'%Y-%m-%d_%H-%M-%S')
